...
function TeacherSignIn() {
    ...
    const data = (e) => {
        e.preventDefault();
        Axios.get(`http://localhost:3000/teachers/?email=${teacher.email}&password=${teacher.password}`)
            .then((res) => {
                if (res.data.length === 1) {
                    const loggedInTeacher = res.data[0];
                    localStorage.setItem('teacher', JSON.stringify(loggedInTeacher));
    ...
    return (
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: '#f0f2f5' }}>
            <div style={{ width: '360px', padding: '20px', borderRadius: '10px', boxShadow: '0 0 10px rgba(0, 0, 0, 0.1)', backgroundColor: '#ffffff' }}>
                <h1 style={{ textAlign: 'center', color: '#333' }}>Teacher Login</h1>
